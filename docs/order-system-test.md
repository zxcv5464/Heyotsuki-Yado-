...
- [ ] 加購選項 `order_limit_quantity` 留空時，該加購不限量。
- [ ] 加購選項 `order_limit_quantity = 0` 時，前台顯示售完且不可勾選。
- [ ] 加購選項 `order_limit_quantity = 1` 時，成功送出一份後剩餘量變成 0。
- [ ] 同一加購選項被多筆購物車品項使用時，送出前會加總檢查。
- [ ] 兩個使用者同時送出最後一份加購時，資料庫只允許一筆成功。
- [ ] 取消或軟刪除訂單不占用加購選項限量。
- [ ] 能力型加購可針對個別人員設定 `order_limit_quantity`。
- [ ] 個別人員限量為 `0` 時，前台選人顯示已滿且不可選。
- [ ] 個別人員限量剩餘為 `1` 時，購物車同一人員同一加購超過 1 份會被前端阻擋。
- [ ] 直接呼叫 `submit_order()` 超過個別人員限量時，資料庫會拒絕。
- [ ] 同時設定加購總量與個人限量時，任一限量不足都不可送出。
