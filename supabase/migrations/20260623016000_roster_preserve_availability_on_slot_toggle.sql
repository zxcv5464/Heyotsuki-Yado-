-- Preserve roster availability when an existing period's shift slots are toggled.
-- Previous save_roster_period deleted and recreated all shift slots, which cascaded
-- roster_availability rows. This version upserts stable slot keys instead.
...
declare
  ...
  slot_keys text[] := array[]::text[];
  slot_key text;
  requirement_role_ids uuid[] := array[]::uuid[];
  requirement_role_id uuid;
begin
  ...
  for slot in select value from jsonb_array_elements(coalesce(p_slots, '[]'::jsonb)) loop
    ...
    slot_key :=
      (slot->>'businessDate') || ':' || coalesce((slot->>'sortOrder')::integer, 0)::text;
    slot_keys := array_append(slot_keys, slot_key);
    ...
    on conflict (period_id, business_date, sort_order) do update
  ...
  delete from public.roster_shift_slots
  where period_id = target_id
    and not (
      (business_date::text || ':' || sort_order::text) = any(slot_keys)
    );
  ...
  for requirement in select value from jsonb_array_elements(coalesce(p_requirements, '[]'::jsonb)) loop
    requirement_role_id := nullif(requirement->>'roleId', '')::uuid;
    requirement_role_ids := array_append(requirement_role_ids, requirement_role_id);
    ...
    on conflict (period_id, role_id) do update
    set role_name_snapshot = excluded.role_name_snapshot,
  ...
  delete from public.roster_period_role_requirements
  where period_id = target_id
    and not (role_id = any(requirement_role_ids));
