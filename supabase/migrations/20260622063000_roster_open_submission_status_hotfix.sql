-- Roster Hotfix: newly created periods always open staff submission.
-- Keep legacy status values for historical compatibility, but only expose
-- open/closed submission states in the administration UI.

alter table public.roster_periods
  alter column status set default 'open';

-- Old create flows produced draft rows without an opening timestamp. Convert
-- only those rows; a draft with submission_open_at was deliberately closed.
update public.roster_periods
set status = 'open',
    submission_open_at = coalesce(submission_open_at, now()),
...
begin
  ...
  for requirement in select value from jsonb_array_elements(coalesce(p_requirements, '[]'::jsonb)) loop
    insert into public.roster_period_role_requirements (
      period_id, role_id, role_name_snapshot, min_staff_count, max_staff_count, sort_order, is_required
    ) values (
      target_id,
