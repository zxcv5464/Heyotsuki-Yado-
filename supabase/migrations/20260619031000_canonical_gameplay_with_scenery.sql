-- v1.0.1 canonical gameplay migration with scenery cards.
-- This file intentionally does not rewrite prior migration history. It replaces
-- the final gameplay/card-pool functions with explicit definitions and keeps
-- internal authoritative game_states.state hidden from direct API access.
...
begin
  ...
  if turns_per_player <= 0 or designation_card_count < player_count * 3 then
  ...
  initial_state := jsonb_build_object(
    ...
    'currentPlayerIndex', public.game_seeded_index(game_seed, 'first-player', player_count),
...
comment on function public.start_game_room(uuid)
is 'Starts a server-authoritative Hanafuda game with seed-derived deck, staff-only designation choices, scenery cards, monthly theme, and first player.';
